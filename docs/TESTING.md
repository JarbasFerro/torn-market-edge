...
22. **Real markup** - fixtures `itemmarket-addlisting.html`, `inventory-real.html`, `bazaar-manage-react.html` and `itemmarket-item-aria.html` mirror Torn's structures as verified from public userscript sources; keep them in sync with those scripts when Torn changes.
23. **Stability** - overlapping scans coalesce to one overlay per row; equipment rows show floors on every sell-side surface and uid-priced copies; untradable items are labelled; the Item Market sell form and a generic Bazaar add fallback are recognised by structure.
