...
run("Item Market browse grid is annotated against the official market value without order-book requests", async (t) => {
  ...
  assert.match(byItem["206"].textContent, /-4\.8% vs value/);
  assert.match(byItem["206"].textContent, /CONSIDER/);
  assert.match(byItem["258"].textContent, /\+20\.0% vs value/);
  assert.match(byItem["258"].textContent, /PASS/);
...
run("Real inventory rows: equipped wrap excluded, hidden tab ignored, data-qty used, weapons skipped", async (t) => {
  ...
  assert.match(xanax.textContent, /Bazaar \$/);
  assert.match(xanax.textContent, /10 owned/, "owned quantity on the line");
...
run("Inventory rows are found through li[data-item] before lazy thumbnails load, and collapsed tabs are skipped", async (t) => {
  ...
  assert.match(xanax.textContent, /Bazaar \$/);
  assert.match(xanax.textContent, /10 owned/);
  const grinder = env.document.querySelector("li[data-item='1509'] .me-inline-analysis");
  assert.match(grinder.textContent, /Bazaar \$/);
